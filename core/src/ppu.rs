...
impl Ppu {
            ...
            HBLANK_MODE => {
                ...
                if self.dots >= DOTS_PER_SCANLINE {
                    ...
                    // self.render_vram_debug(memory_bus); // Option 1: Do it here
                        ...
                        // Transition back to OAM Scan for next visible line
    ...
    pub fn render_vram_debug(&mut self, memory_bus: &MemoryBus) {
        ...
        for tile_idx in 0..NUM_TILES_TO_SHOW {
            ...
            for y_in_tile in 0..8 {
                ...
                if row_addr >= VRAM_START && row_addr < VRAM_END {
                    // Need row_addr+1 too
    ...
    fn render_scanline(&mut self, memory_bus: &mut MemoryBus) {
        ...
        for x in 0..GB_WIDTH as u8 {
            ...
            if bg_win_display_enabled {
                // Only render BG/Win if master enable is on
            ...
            if (self.lcdc & (1 << LCDC_OBJ_ENABLE)) != 0 {
                let (sprite_pixel, sprite_oam_prio, sprite_palette) =
                    self.fetch_sprite_pixel_data(x, y, memory_bus);

                if sprite_pixel != 0 {
                    // Is there a non-transparent sprite pixel here?
                    ...
                    let sprite_wins_priority =
                        !bg_win_display_enabled || (!sprite_oam_prio) || bg_win_is_transparent;
    ...
    fn fetch_bg_pixel_data(
        ...
        let map_base_addr = if (self.lcdc & (1 << LCDC_BG_MAP_AREA)) == 0 {
            0x9800
        } else {
            0x9C00
        };
        ...
        if row_addr < VRAM_START || row_addr + 1 > VRAM_END {
            return (0, false);
    ...
    fn fetch_window_pixel_data(
        ...
        let map_base_addr = if (self.lcdc & (1 << LCDC_WINDOW_MAP_AREA)) == 0 {
            0x9800
        } else {
            0x9C00
        };
    ...
    fn fetch_sprite_pixel_data(
        ...
        let sprite_height = if (self.lcdc & (1 << LCDC_OBJ_SIZE)) != 0 {
            16
        } else {
            8
        };
        ...
        for i in 0..40 {
                ...
                screen_y < effective_y.wrapping_add(sprite_height)
            {
                ...
                if sprite_x != 0 && // sprite_x == 0 means not visible? Check docs. Often true.
                    ...
                    screen_x < effective_x.wrapping_add(8)
                {
                    ...
                    if row_addr < VRAM_START || row_addr + 1 > VRAM_END {
                        continue;
                    } // Skip if OAM points outside VRAM
                    ...
                    if color_index != 0 {
                        ...
                        if sprite_x < highest_prio_sprite_x
                            || (sprite_x == highest_prio_sprite_x
                                && i < highest_prio_sprite_oam_idx)
                        {
                            ...
                            highest_prio_sprite_palette = if palette_num == 0 {
                                memory_bus.read_byte(OBP0_ADDR)
                            } else {
                                memory_bus.read_byte(OBP1_ADDR)
                            };
                        }
                    }
                }
        ...
        (
            highest_prio_sprite_pixel,
            highest_prio_sprite_oam_prio,
            highest_prio_sprite_palette,
        )
    }
}
