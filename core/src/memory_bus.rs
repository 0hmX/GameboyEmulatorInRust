...
impl RtcRegisters {
    ...
    fn update(&mut self) {
        ...
        if days >= 512 { // Day counter wraps around after 511 days
            days %= 512;
...
// --- Joypad State ---
#[derive(Clone, Debug, Default)]
pub struct JoypadState { // Keep private, accessed via methods
...
impl MemoryBus {
    ...
    pub fn load_rom(&mut self, rom_data: &[u8]) {
        if rom_data.len() < 0x150 { // Need at least header size
        ...
        self.num_rom_banks = match rom_size_code {
            ...
            0x08 => 512,
            0x52 => 72,   0x53 => 80,   0x54 => 96, // Less common sizes
        ...
        if rom_data.len() < expected_rom_size {
             println!("Warning: ROM file size ({}) is smaller than expected ({}) based on header.", rom_data.len(), expected_rom_size);
             // Adjust num_rom_banks or just be careful with reads? Let's keep header value for now.
        }
         if rom_data.len() > expected_rom_size {
             println!("Warning: ROM file size ({}) is larger than expected ({}) based on header. Extra data might be ignored by banking.", rom_data.len(), expected_rom_size);
         }
        ...
        if ram_size > 0 && !self.has_ram {
            println!("Warning: Cartridge header RAM size {:02X} but type {:02X} usually lacks RAM.", ram_size_code, cartridge_type_code);
        }
         if ram_size == 0 && self.has_ram && self.mbc_type != MbcType::Mbc3 { // MBC3 might have only Timer/Batt
             println!("Warning: Cartridge header RAM size 00 but type {:02X} expects RAM.", cartridge_type_code);
         }
        ...
        if self.has_ram && ram_size > 0 {
            ...
            if self.num_ram_banks == 0 && ram_size > 0 { self.num_ram_banks = 1; } // Handle 2KB, 8KB cases etc.
        } else {
            ...
            EXT_RAM_START..=EXT_RAM_END => {
                ...
                match self.mbc_type {
                     ...
                     _ => { // Includes NoMbc RAM, Mbc1 RAM, and Mbc3 RAM access
                        if !self.has_ram || self.external_ram.is_empty() { return 0xFF; }
                        if self.current_ram_bank >= self.num_ram_banks { return 0xFF; }
                        ...
                        } else { 0xFF } // Should not happen if bounds checks above are correct
                     }
                }
            }
            ...
            IO_REGISTERS_START..=IO_REGISTERS_END => {
                ...
                match addr {
                    P1_JOYP_ADDR => { // Joypad Read
                        ...
                        if selection_bits & 0x20 == 0 { // Bit 5 Low: Select Action buttons
                            if self.joypad.a      { joypad_value &= 0b1110; } // Bit 0 low if pressed
                            if self.joypad.b      { joypad_value &= 0b1101; } // Bit 1 low if pressed
                            if self.joypad.select { joypad_value &= 0b1011; } // Bit 2 low if pressed
                            if self.joypad.start  { joypad_value &= 0b0111; } // Bit 3 low if pressed
                        }
                        if selection_bits & 0x10 == 0 { // Bit 4 Low: Select Direction buttons
                            if self.joypad.right { joypad_value &= 0b1110; } // Bit 0 low if pressed
                            if self.joypad.left  { joypad_value &= 0b1101; } // Bit 1 low if pressed
                            if self.joypad.up    { joypad_value &= 0b1011; } // Bit 2 low if pressed
                            if self.joypad.down  { joypad_value &= 0b0111; } // Bit 3 low if pressed
                        }
    ...
    pub fn write_byte(&mut self, addr: u16, value: u8) {
        match addr {
            ...
            0x0000..=0x1FFF => { // RAM/RTC Enable (MBC1, MBC3)
                 match self.mbc_type {
                    MbcType::Mbc1 | MbcType::Mbc3 => {
                        if self.has_ram || self.mbc_type == MbcType::Mbc3 { // MBC3 always has RTC chip
            ...
            0x4000..=0x5FFF => { // RAM Bank Number / ROM Bank Upper Bits (MBC1) / RTC Register Select (MBC3)
                 match self.mbc_type {
                     ...
                     MbcType::Mbc3 => {
                         if value <= 0x07 { // Select RAM Bank 0-7
                         ...
                         } else if value >= 0x08 && value <= 0x0C { // Select RTC Register
            ...
            // External RAM / RTC Registers
            EXT_RAM_START..=EXT_RAM_END => {
                if !self.ram_enabled { return; } // Write ignored if disabled
                match self.mbc_type {
                     MbcType::Mbc3 if self.rtc_mapped_register >= 0x08 && self.rtc_mapped_register <= 0x0C => {
                     ...
                     _ => { // RAM access
                        if !self.has_ram || self.external_ram.is_empty() { return; }
                         if self.current_ram_bank >= self.num_ram_banks { return; }
                        let ram_offset = (self.current_ram_bank * EXT_RAM_SIZE) + (addr - EXT_RAM_START) as usize;
                        if ram_offset < self.external_ram.len() {
            ...
            IO_REGISTERS_START..=IO_REGISTERS_END => {
                let offset = (addr - IO_REGISTERS_START) as usize;
                match addr {
                     P1_JOYP_ADDR => { // Joypad Write (only bits 4, 5 are writable)
                         self.io_registers[offset] = (self.io_registers[offset] & 0xCF) | (value & 0x30);
                     }
                     DIV_ADDR => { // DIV - Writing any value resets it to 0
                     ...
                     IF_ADDR => { // Interrupt Flags - Writing 1 clears the flag
                          self.io_registers[offset] &= !value;
                     }
                     LCDC_ADDR | STAT_ADDR | SCY_ADDR | SCX_ADDR | LYC_ADDR |
                     BGP_ADDR | OBP0_ADDR | OBP1_ADDR | WY_ADDR | WX_ADDR => {
                         self.io_registers[offset] = value;
                         // TODO: Notify PPU component of potential change
                     }
                     DMA_ADDR => { // DMA Transfer Trigger
                         self.io_registers[offset] = value;
                         self.perform_dma_transfer(value);
                     }
                     // TODO: Add Sound Register writes (FF10-FF26, FF30-FF3F)
                     _ => { // Default write for unhandled IO regs
                         self.io_registers[offset] = value;
                     }
                 }
            }
    ...
    fn perform_dma_transfer(&mut self, source_high_byte: u8) {
        ...
        for i in 0..OAM_SIZE { // Copy 160 bytes
    ...
    pub fn key_down(&mut self, key: Keycode) {
        ...
        match key {
            ...
            Keycode::Right | Keycode::D => {
                if !self.joypad.right { button_newly_pressed = true; self.joypad.right = true; }
                if p1_reg & 0x10 == 0 { selection_active = true; }
            }
            Keycode::Left | Keycode::A => { // Remap 'A' key to Left
                if !self.joypad.left { button_newly_pressed = true; self.joypad.left = true; }
                 if p1_reg & 0x10 == 0 { selection_active = true; }
            }
            Keycode::Up | Keycode::W => {
                 if !self.joypad.up { button_newly_pressed = true; self.joypad.up = true; }
                 if p1_reg & 0x10 == 0 { selection_active = true; }
            }
            Keycode::Down | Keycode::S => {
                 if !self.joypad.down { button_newly_pressed = true; self.joypad.down = true; }
                 if p1_reg & 0x10 == 0 { selection_active = true; }
            }
            // Actions (Check bit 5 of P1 register)
            Keycode::Z | Keycode::J => { // GB 'A' button
                 if !self.joypad.a { button_newly_pressed = true; self.joypad.a = true; }
                 if p1_reg & 0x20 == 0 { selection_active = true; }
            }
            Keycode::X | Keycode::K => { // GB 'B' button
                 if !self.joypad.b { button_newly_pressed = true; self.joypad.b = true; }
                 if p1_reg & 0x20 == 0 { selection_active = true; }
            }
            Keycode::Backspace | Keycode::RShift => { // GB 'Select' button
                 if !self.joypad.select { button_newly_pressed = true; self.joypad.select = true; }
                 if p1_reg & 0x20 == 0 { selection_active = true; }
            }
            Keycode::Return | Keycode::Space => { // GB 'Start' button
                 if !self.joypad.start { button_newly_pressed = true; self.joypad.start = true; }
                 if p1_reg & 0x20 == 0 { selection_active = true; }
            }
            _ => {} // Ignore other keys
        }
